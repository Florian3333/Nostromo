...
def read_data(liste):
    ...
    fichier_xml = str(chosing_month) + str(liste[which_file-1])  
    return fichier_xml
...
def pdf_report(current_date, modeles, nombres, aa, current_time):
    ...
    titre = f"Budget {aa} issued the {str(current_date)} at {str(current_time)}" ######################### mettre date du fichier xml
    ...
    pdf.output(f"budget{aa} issued the {str(current_date)} at {str(current_time)}.pdf") ########################### changer date du fichier xml
